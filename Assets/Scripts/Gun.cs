...
public class Gun : MonoBehaviourPun, IPunObservable {
    ...
    {
        return new Vector3(mousePositon.position.x, transform.position.y, mousePositon.position.z);
    }
    // 호스트에서 실행되는, 실제 발사 처리
    [PunRPC]
    private void ShotProcessOnServer() {
        ...
        if (Physics.Raycast(fireTransform.position,
            (Rotation() - transform.position), out hit, fireDistance))
        {
            // 레이가 어떤 물체와 충돌한 경우
